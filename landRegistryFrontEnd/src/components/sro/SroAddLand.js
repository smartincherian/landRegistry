...
function SroAddLand() {
  ...
  const searchButtonHandler = async () => {
    ...
    const response = await fetch(`http://localhost:5000/api/findLand`, {
      method: "post",
  ...
  const verifyButtonHandler = async () => {
    ...
    setisLoading(true);
    try {
      const response = await fetch(`http://localhost:5000/api/addLand`, {
        method: "post",
        body: JSON.stringify({
          ...
          areaAcres: searchedLandData[0].areaAcres,
          areaCent: searchedLandData[0].areaCent,
